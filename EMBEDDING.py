...
def embed(images_DIR_local=images_DIR, embedded_DIR_local=embedded_DIR, watermark_DIR_local=watermark_DIR):
    """
    Embeds a watermark into multiple images using multithreading.

    This function processes all images in the specified directory, embeds a watermark
    into each image, and saves the result in the embedded directory. It uses
    multithreading to improve performance.

    Parameters:
    images_DIR_local (str): Path to the directory containing original images.
                            Defaults to the global 'images_DIR'.
    embedded_DIR_local (str): Path to the directory where watermarked images will be saved.
                              Defaults to the global 'embedded_DIR'.
    watermark_DIR_local (str): Path to the watermark image file.
                               Defaults to the global 'watermark_DIR'.
